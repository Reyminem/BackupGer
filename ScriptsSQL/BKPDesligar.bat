...
set "backup_dir=C:\BKP_1.2\Backup\Desligar"
...
for /f "usebackq tokens=1" %%d in (`sqlcmd -S !server! -U !user! -P !password! -Q "EXEC sp_databases" -h-1`) do (
    ...
    if /i not "!db_name!"=="master" if /i not "!db_name!"=="tempdb" if /i not "!db_name!"=="model" if /i not "!db_name!"=="msdb" if /i not "!db_name!"=="ReportServer" if /i not "!db_name!"=="ReportServerTempDB" (
        set "backup_file=!backup_dir!\BKPDesligarSQLServer_!db_name!.bak"
        ...
        echo Backup do banco de dados !db_name! concluído: !backup_file!
        echo.
    )
)

echo Compactando arquivos .bak em Desligar.zip...
cd "C:\BKP_1.2\Backup\Desligar"
"C:\Program Files\7-Zip\7z.exe" a -tzip "C:\BKP_1.2\Backup\Desligar\BKPDesligarSQLServer.zip" "*.bak"
copy "C:\BKP_1.2\Backup\Desligar\BKPDesligarSQLServer.zip" "C:\Program Files (x86)\12informatica\BackupDrive\Desligar"
...
set "backupFile=C:\BKP_1.2\Backup\Desligar"
