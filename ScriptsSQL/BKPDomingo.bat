...
set "backup_dir=C:\BKP_1.2\Backup\Domingo"
...
for /f "usebackq tokens=1" %%d in (`sqlcmd -S !server! -U !user! -P !password! -Q "EXEC sp_databases" -h-1`) do (
    ...
    if /i not "!db_name!"=="master" if /i not "!db_name!"=="tempdb" if /i not "!db_name!"=="model" if /i not "!db_name!"=="msdb" if /i not "!db_name!"=="ReportServer" if /i not "!db_name!"=="ReportServerTempDB" (
        set "backup_file=!backup_dir!\BKPDomingo_!db_name!.bak"
...
echo Compactando arquivos .bak em Domingo.zip...
cd "C:\BKP_1.2\Backup\Domingo"
"C:\Program Files\7-Zip\7z.exe" a -tzip "C:\BKP_1.2\Backup\Domingo\DomingoSQLServer.zip" "*.bak"
copy "C:\BKP_1.2\Backup\Domingo\DomingoSQLServer.zip" "C:\Program Files (x86)\12informatica\BackupDrive\Domingo"
...
set "backupFile=C:\BKP_1.2\Backup\Domingo"
