...
for /f "usebackq" %%d in (`mysql -h %host% -u %user% -p%password% -e "SHOW DATABASES;"`) do (
  if /i not "%%d"=="information_schema" if /i not "%%d"=="mysql" if /i not "%%d"=="performance_schema" if /i not "%%d"=="database" if /i not "%%d"=="test" (
    set "backup_file=%backup_dir%\BKPDesligarMySQL_%%d.sql"
...
"C:\Program Files\7-Zip\7z.exe" a -tzip "C:\BKP_1.2\Backup\Desligar\BKPDesligarMySQL.zip" "*.sql"
copy "C:\BKP_1.2\Backup\Desligar\BKPDesligarMySQL.zip" "C:\Program Files (x86)\12informatica\BackupDrive\Desligar"
...
set "backupFile=C:\BKP_1.2\Backup"
